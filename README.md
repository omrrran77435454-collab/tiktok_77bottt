...
```
الصفحة الرئيسية
   ↓ تسجيل الدخول بحساب Google (الجلسة تبقى بعد إغلاق المتصفّح)
ربط حساب Telegram (زر واحد — بدون كتابة أي معلومة يدوياً)
   ↓ زر «اشترك في القناة» ثم زر «تحقق من الاشتراك»
تهيئة الحساب مرة واحدة: معلم أو طالب ← المرحلة ← الصف ← المسار ← المواد
   ↓
لوحة مخصّصة حسب الدور → أداة أو الجدول الأسبوعي → معاينة حيّة
...
| الأداة | ماذا تفعل |
|---|---|
| جدولي الأسبوعي | جدول حصص مرتّب حسب اليوم + نصيب كل مادة من الأسبوع |
| خطة المذاكرة | توزيع وقتك المتاح على المواد توزيعاً مرجَّحاً بالأولوية |
| منظّم الواجبات | ترتيب الواجبات حسب الأقرب تسليماً وإبراز المتأخّر والمستحقّ اليوم |
| خطة ما قبل الاختبار | توزيع الدروس على الأيام المتبقّية — الأضعف أولاً، واليوم الأخير مراجعة |

بالإضافة إلى **الجدول الأسبوعي** (صفحة كاملة لا أداة): إنشاء وتعديل وحذف
ونسخ الحصص، نسخ اليوم السابق، ومسح يوم أو أسبوع، مع حالة تحضير ومتابعة لكل حصة.

لوحة إدارة محمية على `/admin` فيها تبويبان: إحصاءات الاستخدام العامة (بلا أي
بيانات طلاب)، وإدارة المحتوى (الأدوات والأقسام والمواد والمراحل والصفوف
والمسارات) مع سجل يوثّق كل إجراء إداري.

### فصل الدورين

| المفهوم | العمود | القيم | الغرض |
|---|---|---|---|
| صلاحية النظام | `users.role` | `user` / `admin` | من يدخل لوحة الإدارة — يُفحص في الخادم دائماً |
| دور التجربة | `profiles.role` | `teacher` / `student` | أي لوحة وأي أدوات تُعرض |

الفصل مقصود: الإدمن يستطيع استخدام المنصّة كمعلم دون أن يفقد صلاحيته،
ولا يستطيع أي عميل رفع صلاحيته بتعديل ملفه الشخصي.
...
| الملف | ماذا يحرس |
|---|---|
| `auth-boot` | بقاء الجلسة (IndexedDB ثم localStorage، ولا تخزين جلسة) وحالة الإقلاع |
| `telegram-state` | آلة حالات الربط: لا زر تحقّق بلا زر اشتراك |
| `telegram-messages` | رابط القناة في نهاية كل رسالة بوت، وبلا أي سرّ |
| `platform.integration` | التهيئة، ترشيح الأدوات، CRUD الجدول، وصلاحيات الإدارة |
| `api.integration` | المصادقة وبوابة تيليجرام والتحقّق والإحصاءات |
| `student-tools` | منطق أدوات الطالب الأربع |
| `document-model` | كل أداة تُنتج مستنداً صالحاً بجداول فعلية |
...
│   │   ├── telegram-messages.ts مصدر واحد لرابط القناة ونصوص البوت
│   │   ├── catalog-repo.ts    البنية التعليمية والملفّات وترشيح الأدوات
│   │   ├── schedule-repo.ts   الجدول الأسبوعي
│   │   ├── admin-repo.ts      إدارة المحتوى + سجل الإجراءات
│   │   └── …                  router, http, crypto, telegram, repo
│   └── routes/                me, profile, schedule, telegram, analytics, admin
├── src/                       الواجهة (React)
...
│   ├── lib/                   firebase, auth, auth-persistence, api, session,
│   │                          education, telegram-state, useSchedule, useTools,
│   │                          useCatalog, saved-work, colors, storage, analytics
