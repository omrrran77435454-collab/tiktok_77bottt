...
- **Covers are the owner's photographic artwork.** All eleven covers supplied by the owner replace the editorial vector covers this library originally shipped. Each is placed full-bleed on page 1 of both files at exactly A4 (210 × 297 mm), with no page margin and no footer over the artwork. The replacement changed page 1 only: every page count, page number, page order, the copyright page, the per-page footer and all internal content are identical to the verified builds recorded above. Each of the twenty-two files was re-checked after the swap — artwork present on page 1, title legible, copyright page intact on page 2, file opens, Telegram link annotation unchanged.
