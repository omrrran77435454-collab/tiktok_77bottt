...
| Official sources checked | platform.claude.com (models overview, prompting best practices), code.claude.com (Claude Code overview + docs index), support.claude.com (projects, artifacts, memory, web search, research, file uploads, plans and limits) |
...
| RTL (DOCX) | PASS — `w:bidi` set on the section and on 482 paragraphs; verified by re-parsing the saved file |
| Right alignment | PASS — verified visually on every rendered page |
| Arabic shaping | PASS — letters joined, no isolated forms; verified on the full-page renders of all 45 PDF pages |
...
| Copy test | PASS — all 1611 distinct Arabic words of the canonical source were found in the extracted text layer; sampled from the beginning, middle and end of the document, and from prompt boxes |
...
| Telegram link present | PASS — appears 47 times in the PDF text layer and 3 times in the DOCX |
...
| Pages visually inspected | 45/45 PDF pages and 40/40 DOCX pages — every page was rendered to PNG and reviewed as a contact sheet, with individual pages opened at full size |
...
- IBM Plex Sans Arabic puts 241 ligatures in the required rlig feature (not only lam-alef). Each rendered one glyph mapped to two characters in logical order inside an otherwise visual-order run, so 14 of 24 sampled words could not be found by PDF search and العربي copied as العريب. Fixed by rewriting the ToUnicode CMap of every multi-character Arabic mapping (fix_tounicode.py); the sample went from 14 failures to 0.
- The table of contents used a CSS float for page numbers, which escaped the text area and printed one number outside the left margin. Rebuilt as a flex row with leader dots; page numbers now sit inside the margin and come from target-counter.
- Arabic ready-to-copy prompts were being rendered inside LTR code blocks, which reversed their variables. A dedicated RTL prompt-box block type was added and 15 blocks were converted; code blocks are now reserved for real terminal and source code.
- Lines ending with a variable had the closing bracket relocated on copy. The build now moves a trailing variable onto its own line and strips punctuation immediately after a closing bracket.
- Digits inside variables ([الخيار 1]) broke bracket order; replaced with Arabic ordinals and a QA gate now rejects any variable containing digits.
- A malformed callout (a string where a list was expected) rendered one Arabic letter per line across two pages. The engine now validates every block and fails the build instead of rendering it.
- Word could not be rendered at first: LibreOffice Writer was missing from the image, and WordprocessingML children were being appended in the wrong schema order. Both fixed; the DOCX now opens and renders, and every page was inspected.
