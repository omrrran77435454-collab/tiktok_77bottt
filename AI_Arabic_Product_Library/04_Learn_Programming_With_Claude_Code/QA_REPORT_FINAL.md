# QA_REPORT_FINAL — تعلم البرمجة مع Claude Code

| Field | Result |
|---|---|
| Product | تعلم البرمجة مع Claude Code |
| Files | `Learn_Programming_With_Claude_Code_Arabic_Final.docx` · `Learn_Programming_With_Claude_Code_Arabic_Final.pdf` |
| Research verified at | 6 سبتمبر 2026 |
| Official sources checked | code.claude.com/docs (permission modes, memory/CLAUDE.md, sessions) for the tooling chapters, and python.org/downloads for the stable Python release used across the lessons |
...
| DOCX pages | 64 (rendered through LibreOffice Writer) |
| PDF pages | 63 |
...
| RTL (DOCX) | PASS — `w:bidi` set on the section and on 1184 paragraphs; verified by re-parsing the saved file |
| Right alignment | PASS — verified visually on every rendered page |
| Arabic shaping | PASS — letters joined, no isolated forms; verified on the full-page renders of all 63 PDF pages |
...
| Variables `[…]` | PASS — 21 distinct variables, all intact in the text layer and findable by PDF search |
...
| TOC links (PDF) | PASS — 410 internal link annotations, every one resolving to a page inside the document |
...
| Copy test | PASS — all 1632 distinct Arabic words of the canonical source were found in the extracted text layer; sampled from the beginning, middle and end of the document, and from prompt boxes |
...
| Telegram link present | PASS — appears 65 times in the PDF text layer and 3 times in the DOCX |
...
| Pages visually inspected | 63/63 PDF pages and 64/64 DOCX pages — every page was rendered to PNG and reviewed as a contact sheet, with individual pages opened at full size |
...
- The digit-in-variable QA gate fired on Python list literals such as [1, 2, 3] and [90, 85, 78] inside ASCII code blocks. Those are not template variables and are not affected by bidi reordering, so the check was narrowed to brackets that hold Arabic text. Re-verified against products 01 to 03 after the change.
- Every lesson is generated from one LESSON template, so the Learn / Try / Review / Fix / Test / Improve structure is identical in all 17 lessons and cannot drift between them.
- All review prompts share a fixed rules block that forbids writing the full solution, requires explaining why an error happened rather than only what it was, and ends with a question. The teaching CLAUDE.md in chapter two enforces the same contract at session level.
- Code examples are ASCII-only, which the engine now enforces, so no lesson can ship a code block whose Arabic content would reorder against the block direction.
...
- **Lesson code was not executed in this environment.** The Python examples are short and standard, and were reviewed by reading, but no test run was performed against a Python interpreter as part of the build. Treat the outputs described in the lessons as the intended behaviour to verify while you work through them — which is exactly what the read-then-predict-then-run exercises ask you to do.
